...
//_float3 CTransform::Get_EulerRotation()
//{
...
//
//	// 쿼터니언으로 변환
//	_vector qRot = XMQuaternionRotationMatrix(matRotation);
//
//	// 오일러 각도로 변환 (RollPitchYaw 순)
//	_float pitch, yaw, roll;
//
//	_float4 q;
//	XMStoreFloat4(&q, qRot);
//
//	// 직접 수학적으로 변환
...
//
//	_float sinp = 2.f * (q.w * q.y - q.z * q.x);
//	if (std::abs(sinp) >= 1.f)
//		pitch = std::copysign(DirectX::XM_PIDIV2, sinp); // 90도
//	else
//		pitch = std::asin(sinp);
//
...
//
//
//void CTransform::Set_EulerRotation(const _float3& vEulerDeg)
//{
//	_float radX = XMConvertToRadians(vEulerDeg.x);
//	_float radY = XMConvertToRadians(vEulerDeg.y);
//	_float radZ = XMConvertToRadians(vEulerDeg.z);
//
//	_vector qRot = XMQuaternionRotationRollPitchYaw(radX, radY, radZ);
//
//	// 기본 축 기준 회전 적용
//	_vector vRight = XMVector3Rotate(XMVectorSet(1.f, 0.f, 0.f, 0.f), qRot);
//	_vector vUp = XMVector3Rotate(XMVectorSet(0.f, 1.f, 0.f, 0.f), qRot);
//	_vector vLook = XMVector3Rotate(XMVectorSet(0.f, 0.f, 1.f, 0.f), qRot);
//
