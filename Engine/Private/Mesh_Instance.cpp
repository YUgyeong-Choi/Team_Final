...
{
	_uint NameLength = {};
	ifs.read(reinterpret_cast<char*>(&NameLength), sizeof(_uint));			// ¸Þ½¬ ÀÌ¸§ ±æÀÌ 
	ifs.read(reinterpret_cast<char*>(m_szName), NameLength);				// ¸Þ½¬ ÀÌ¸§
	ifs.read(reinterpret_cast<char*>(&m_iMaterialIndex), sizeof(_uint));	// ¸ÓÅ×¸®¾ó ÀÎµ¦½º
	ifs.read(reinterpret_cast<char*>(&m_iNumVertices), sizeof(_uint));		// ¹öÅØ½º ¸î°³
	ifs.read(reinterpret_cast<char*>(&m_iNumIndices), sizeof(_uint));		// ÀÎµ¦½º ¸î°³
	m_iNumVertexBuffers = 1;
	m_iIndexStride = sizeof(_uint);
	m_eIndexFormat = DXGI_FORMAT_R32_UINT;
	m_ePrimitiveTopology = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	...
	ifs.read(reinterpret_cast<char*>(m_pIndices), sizeof(_uint) * m_iNumIndices);			// ¹öÅØ½º ±¸Á¶Ã¼ ¹è¿­
