...
{
	for (auto& [key, value] : m_Sounds)
		value->StopAll(); // Stop() 대신 StopAll() 사용
}

void CSoundController::StopAllSpecific(const string& tags)
{
	auto pSoundCore = Find_Sound(tags);
	if (pSoundCore)
		pSoundCore->StopAll();
}
...
void CSoundController::Update3DPosition(_float3& vPos)
{
	for (auto& [key, value] : m_Sounds)
	{
