...
class CQuadTree_Manager final : public CBase
{
...
public:
	...
	struct Frustum
	{
		...
		FrustumHit QuadIsInAABB(const _float3& bmin, const _float3& bmax) const;
		void QuadBuild(const _matrix& view, const _matrix& proj);
	};

private:
	CQuadTree_Manager(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	virtual ~CQuadTree_Manager() = default;


public:
	// 초기화
	HRESULT Initialize();
	HRESULT Ready_QuadTree(const vector<AABBBOX>& staticBounds, const map<Handle, _uint>& handleToIndex);
	...
	int ClassifyFullyContained(const AABBBOX child[4], const AABBBOX& obj) const;
...
public:
	static CQuadTree_Manager* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
