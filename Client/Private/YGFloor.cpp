...
CYGFloor::CYGFloor(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CGameObject{ pDevice, pContext }
{
}

CYGFloor::CYGFloor(const CYGFloor& Prototype)
    : CGameObject(Prototype)
{
}

HRESULT CYGFloor::Initialize_Prototype()
{
    return S_OK;
}
...
void CYGFloor::Update(_float fTimeDelta)
{

}

void CYGFloor::Late_Update(_float fTimeDelta)
{
...
HRESULT CYGFloor::Bind_ShaderResources()
{
...
HRESULT CYGFloor::Ready_Components()
{
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(ENUM_CLASS(LEVEL::YG), TEXT("Prototype_Component_Model_Floor"),TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModelCom))))
		return E_FAIL;
...
HRESULT CYGFloor::Ready_Collider()
{
...
CYGFloor* CYGFloor::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CYGFloor* pInstance = new CYGFloor(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CYGFloor");
...
CGameObject* CYGFloor::Clone(void* pArg)
{
	CYGFloor* pInstance = new CYGFloor(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CYGFloor");
		Safe_Release(pInstance);
	}
